import { draw_circle, draw_history, draw_graph } from "./include/gas.js";
const max = 100; // m³, empfohlener Monatsverbrauch laut BDEW

d3.json("../assets/data/gas_usage.json").then(data => {
    data.forEach(d => {
    ...
    for (let i = 0; i < sortedMonthlyData.length; i++) {
        ...
        for (let n = 0; n < currentMonthData.entries.length; n++) {
            ...
            console.log(`Monat: ${currentMonthData.month}, Datum: ${currentEntry.date.toLocaleDateString('de-DE')}, Wöchentliche Differenz: ${weeklyDiff.toFixed(1)} m³, Kumulativ: ${runningTotal.toFixed(1)} m³`);
            detailedDiffs.push({
    ...
    d3.select("#gas-value").text(`${current.toFixed(0)} m³`);
